...
export class DifficultySettingsDialogComponent extends WithDestroy() implements OnInit {
  ...
  inputFields = {
    rangeModifier: {
      ...
      controlMeta: new ControlMetaNumber(0.1, 100, 5, 'x', 'Multiplier applied to antenna strengths'),
    },
    dsnModifier: {
      label: 'DSN Modifier',
      control: new FormControl(this.data.dsnModifier ?? 1, [Validators.required, Validators.min(0), Validators.max(100)]),
      controlMeta: new ControlMetaNumber(0, 100, 5, 'x', 'Multiplier applied to the strength of the DSN'),
    },
  } as InputFields;
