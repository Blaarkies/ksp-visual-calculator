...
export class PartsSelectorComponent extends WithDestroy() implements OnDestroy {
  ...
  constructor(private cacheService: StockEntitiesCacheService,
              private isruService: MiningBaseService) {
    super();
    ...
    //   take(1),
    //   delay(500),
    //   tap(parts => {let splits = parts.splitFilter(p => {let cat = p.value.category;return cat === 'isru' ? 0 : cat === 'drill' ? 1 : cat === 'radiator' ? 2 : cat === 'battery' ? 3 : cat === 'solar-panel' ? 4 : cat === 'resource-tank' ? 5 : cat === 'fuel-cell' ? 6 : cat === 'rtg' ? 7 : 8;});
    //     let newEntries: ControlItem<CraftPart, number>[] = [splits[0][0], splits[1][1], splits[2][4], splits[3][3], splits[4][8], splits[5][26], splits[6][1],/* splits[7][0], */].map(p => ({control: new FormControl(1), value: p.value}));
